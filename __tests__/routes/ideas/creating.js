...
import IdeaService, { mockCreateIdea } from '../../../src/services/ideas/idea.js';
...
describe('Ideas - Create', () => {
    ...
    it('should create an idea and return a 200 response', async (done) => {
        try {
            let expectedIdeaValue = new IdeaModel(testIdea);
            expectedIdeaValue.id = new ObjectID();
            mockCreateIdea.mockResolvedValue(expectedIdeaValue);
            let response = await request.post(`${ideasUrl}`)
    ...
    it('should return a status 400 response when creating an idea with missing parameters', async (done) => {
        try {
            let response = await request.post(`${ideasUrl}`)
                .set('Content-Type', 'application/json')
                .set('x-access-token', 'Bearer ' + testUserModel.accessToken)
